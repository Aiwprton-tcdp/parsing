import { Builder, Browser, Capabilities, ThenableWebDriver } from 'selenium-webdriver';
import { Options } from 'selenium-webdriver/firefox.js';

export class Tournaments {
	private readonly url: string | undefined = process.env.KHL_URL;
	private results: Array<any> | any; // в Array<any> вместо any нужен тип
	...
	async parsePage(): Promise<string | false> {
		if (!this.url) {
			const emptyEnvErrorText: string = 'Ошибка: файл .env не заполнен';
	...
	async parseResults(): Promise<string | any> {
		const result = await this.parsePage();
		return result ? this.results : result;
	}
